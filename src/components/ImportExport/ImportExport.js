...
class ImportExport extends Component {
      ...
      this.setState(
        {
          ...
          entries: upload.serviceRegistryEntries,
          authRules: upload.authRules,
          consumers: upload.consumption
        },
        () => {
          ...
          new Promise((resolve) => {
            ...
            .then(() =>
              this.importAuthRules(
                this.state.authRules,
                this.props.autoCompleteData.interfaceList,
                this.props.systems,
                this.props.services
              )
            )
  ...
  deleteSREntries = (entries) => {
    entries.data
      .filter((entry) => {
        return (
          entry.provider.systemName !== 'orchestrator' &&
  ...
  deleteServices = (services) => {
    return services
      .filter((service) => {
        return (
          service.serviceDefinition !== 'gw-public-key' &&
  ...
  deleteSystems = (systems) => {
    return systems
      .filter((system) => {
        return (
          system.systemName !== 'orchestrator' &&
  ...
  importAuthRules = (rules, interfaceList, systemList, serviceList) => {
    ...
    for (const rule of rules) {
      const lProviders = rule.providers.map((systemName) =>
        systemName.toLowerCase()
      )
      const authData = {
        consumerSystem: systemList.filter((system) => {
          return system.systemName === rule.consumer.toLowerCase()
        })[0],
        service: serviceList.filter((service) => {
          return (
            service.serviceDefinition === rule.serviceDefinition.toLowerCase()
          )
        })[0],
        interfaces: interfaceList.filter((iface) =>
          rule.interfaces.includes(iface.value)
        ),
        providerList: systemList.filter((system) =>
          lProviders.includes(system.systemName)
        )
      }
  ...
  onExportClick = () => {
    const result = new Promise((resolve) => {
      ...
      .then(() => {
        const authRulesHelperArray = this.props.authRules.map(
          ({
            consumerSystem,
            providerSystem,
            interfaces,
        ...
        authRulesHelperArray.map((rule) => {
          if (
            !authRulesHelperObject[
              rule.consumer + rule.interfaces.join() + rule.serviceDefinition
            ]
          ) {
            authRulesHelperObject[
              rule.consumer + rule.interfaces.join() + rule.serviceDefinition
        ...
        const exportObject = {
          ...
          serviceRegistryEntries: this.props.entries.data
            .filter((entry) => {
              return (
                entry.provider.systemName !== 'orchestrator' &&
                entry.provider.systemName !== 'authorization' &&
            ...
            .map((entry) => {
              ...
              entry.serviceDefinition =
                entry.serviceDefinition.serviceDefinition
              delete entry.provider.createdAt
              delete entry.provider.updatedAt
              entry.interfaces = entry.interfaces.map(
...
function mapStateToProps(state) {
  ...
  return {
    autoCompleteData: services.autoCompleteData,
